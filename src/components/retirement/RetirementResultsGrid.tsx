...
function getRetirementCapitalTone(plan: RetirementPlan) {
  if (plan.gap >= 0) {
    return plan.gap >= plan.requiredCapital * 0.15 ? 'metric-card--excellent' : 'metric-card--good';
  }
...
function getRunwayTone(lastsThroughPlan: boolean, inputs: RetirementInputs, plan: RetirementPlan) {
  ...
  return yearsShort >= 7 ? 'metric-card--danger' : 'metric-card--warning';
}
  ...
  return (
    <Card as="section" className="results-panel retirement-results">
      ...
      <div className="results-panel__grid">
        <Card as="article" className={`metric-card ${getRetirementCapitalTone(plan)}`}>
        ...
        <Card as="article" className={`metric-card ${getGapTone(plan)}`}>
        ...
        <Card as="article" className={`metric-card ${getRunwayTone(lastsThroughPlan, inputs, plan)}`}>
